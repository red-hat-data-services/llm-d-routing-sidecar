...
# Defaults
PROJECT_NAME ?= llm-d-routing-sidecar
DEV_VERSION ?= 0.0.1
PROD_VERSION ?= 0.0.0
...
kind-prereq:
	...
	@kind get clusters | grep -q 'kind' || { \
		...
		echo "kind create cluster --config test/e2e/kind-config.yaml" \
		exit 1; \
	}
	kubectl config use-context kind-kind

.PHONY: kind-llm-d-sim-image-build
	@hack/docker-build-llm-d-sim.sh

.PHONY: kind-e2e-test-no-install
kind-e2e-test-no-install:
	@go test ./test/e2e/ -v -ginkgo.v

.PHONY: kind-e2e-test
kind-e2e-test: kind-prereq generate format vet ## Run the e2e tests in kind. Expected an isolated environment using Kind.
kind-e2e-test: kind-install kind-e2e-test-no-install kind-uninstall

.PHONY: kind-load-images
kind-load-images: IMG=llm-d-routing-sidecar:v0.0.1
kind-load-images: LLM_D_SIM_IMG=llm-d-sim:v0.0.1
kind-load-images: image-build kind-llm-d-sim-image-build ## Load images into the local Kind cluster specified in ~/.kube/config.
	kind load docker-image ${IMG}
	kind load docker-image ${LLM_D_SIM_IMG}

.PHONY: kind-install
kind-install: kind-prereq kind-load-images ## Install testing configurations into the local Kind cluster specified in ~/.kube/config.
	kustomize build test/config/overlays/kind | kubectl apply -f -

.PHONY: kind-uninstall
kind-uninstall: ## Uninstall configurations from the local Kind cluster specified in ~/.kube/config.
	kustomize build test/config/overlays/kind | kubectl delete  --ignore-not-found=true -f -
